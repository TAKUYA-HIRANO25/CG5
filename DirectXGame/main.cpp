...
int WINAPI WinMain(_In_ HINSTANCE, _In_opt_ HINSTANCE, _In_ LPSTR, _In_ int) {
	...
	// エンジンの初期化
	KamataEngine::Initialize(L"LE3C_20_ヒラノ_タクヤ_CG5");
...
#pragma region RenderTextureResourceの作成
	// Resource生成
	...
	// DepthStencilTextureResourceの設定
	...
	// DSVHeapの先頭にDSVを作る
	device->CreateShaderResourceView(renderTextureResource, &srvDesc, srvHandeleCPU);
#pragma endregion
...
ID3D12Resource* CreateRenderTextureResource(ID3D12Device* device, uint32_t width, uint32_t height, DXGI_FORMAT clearFormat, const FLOAT* clearColor) {
	...
	[[maybe_unused]] HRESULT hr = device->CreateCommittedResource(
		...
		D3D12_HEAP_FLAG_NONE,   //Heapｎ特殊な設定
...
ID3D12Resource* CreateDepthStencilTextureResource(ID3D12Device* device, int32_t width, int32_t height) {
	...
	// 3, Resourceの生成
	ID3D12Resource* resource = nullptr;
	[[maybe_unused]] HRESULT hr = device->CreateCommittedResource(&heapProperties, D3D12_HEAP_FLAG_NONE, &resourceDesc, D3D12_RESOURCE_STATE_DEPTH_WRITE, &depthClearValue, IID_PPV_ARGS(&resource));
